  ...
  if (signupVal !== null) {
    ...
    zip.addEventListener('blur', () => {
      if (zip.value === '' || zip.value === null) {
        zip.className = 'form-control is-invalid';
        zip.setCustomValidity('Please enter your zip code');
      } else if (zipRegex.test(zip.value) === false) {
        zip.className = 'form-control is-invalid';
        zip.setCustomValidity('Please enter a valid zip code\nIt should be 5 digits long');
      } else {
        zip.className = 'form-control is-valid';
    ...
    bttnSignup.addEventListener('click', event => {
      ...
      if (fName.value !== '' && lName.value !== '' && email.value !== '' && zip.value  !== '' && pass.value !== '' && confirmPass.value !== '' && pass.value === confirmPass.value) {
